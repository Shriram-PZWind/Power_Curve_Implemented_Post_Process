...
def execution_pipeline():
    ...
    if not in_folder:
        validation_errors.append(
            "- 'input folder = ...' is missing or undefined in paths.txt"
        )
    elif not os.path.isdir(os.path.abspath(in_folder)):
        validation_errors.append(
            f"- Input folder path specified does not exist:\n  {in_folder}"
        )

    if not out_folder:
        validation_errors.append(
            "- 'output folder = ...' is missing or undefined in paths.txt"
        )

    if not lc_path:
        validation_errors.append(
            "- 'lcpostproc = ...' path is missing or undefined in paths.txt"
        )
    elif not os.path.isfile(os.path.abspath(lc_path)):
        validation_errors.append(
            f"- LC PostProcess text file specified does not exist:\n  {lc_path}"
        )
    ...
    if sensor_path_from_txt:
        ...
        if os.path.exists(os.path.abspath(sensor_path_from_txt)):
            ...
            print(f"  {sensor_path_from_txt}")
            ans = input(
                "Should we proceed based on the folder input (create/overwrite there)? (y/n): "
            )
            if ans.strip().lower() == "y":
                final_sensor_path = fallback_sensor_path
            else:
    ...
    manage_sensor_list.merge_sensor_list(
        os.path.abspath(final_sensor_path), user_folder
    )
