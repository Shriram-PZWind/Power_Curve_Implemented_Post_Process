...
def write_summary_sta(output_path, file_names, sensor_cols,
                      all_extreme, all_del, m_values,
                      header=None):
    """
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for matrix_name in matrices:
            ...
            for fname in file_names:
                ...
                for col in sensor_cols:
                    if is_stat:
                        val = all_extreme.get(fname, {}).get(col, {}).get(matrix_name, 0.0)
                    else:
...
def write_rfc_file(output_path, bin_edges, counts_20yr, sensor_col,
                   del_rfc=None, lifetime_years=None, neq=None, m_values=None):
    """
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        if del_rfc is not None and m_values is not None:
            ...
            f.write('#\n')
        f.write(f"# {'BinRange (min - max)':>{rw}} {'BinCenter':>{cw}} "
                f"{'CycleCount_lifetime':>{cw}}\n")
        f.write('# ' + '-' * (rw + 2 * cw + 4) + '\n')
        for i in range(len(center)):
            bin_range_str = f"{low[i]:.6g} - {high[i]:.6g}"
            f.write(f"  {bin_range_str:>{rw}} {center[i]:>{cw}.6g} "
                    f"{counts_20yr[i]:>{cw}.6g}\n")


def write_markov_file(output_path, bin_edges_range, bin_edges_mean, matrix_lifetime, sensor_col):
    """
...
def write_ldd_file(output_path, bin_edges_level, time_at_level, revs_at_level, sensor_col,
                   del_ldd=None, lifetime_years=None, neq_time=None, m_values=None):
    """
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        if del_ldd is not None and m_values is not None:
            f.write(f'# Lifetime  : {lifetime_years:.0f} years\n')
            f.write(f'# Neq_time  : {neq_time:.3g} s  (full lifetime, 1Hz assumption)\n')
            f.write('#\n')
            m_hdr = ''.join(f"{f'DEL_m{fmt_slope(m)}':>{cw}}" for m in m_values)
            ...
            f.write(f"# {'[same units as sensor]':>{rw-2}}" + m_val + '\n')
            f.write('#\n')
        f.write(f"# {'LevelBinRange (min - max)':>{rw}} {'LevelBinMean':>{cw}} "
                f"{'CumulTime_s':>{cw}}\n")
        f.write('# ' + '-' * (rw + 2 * cw + 4) + '\n')
        for i in range(len(center)):
            bin_range_str = f"{low[i]:.6g} - {high[i]:.6g}"
...
def write_lrd_file(output_path, bin_edges_level, time_at_level, revs_at_level, sensor_col,
                   del_lrd=None, lifetime_years=None, neq_rev=None, m_values=None):
    """
...
def write_ext_file(output_path, sensor_col, stat,
                   ranked_plf, ranked_noplf):
    """
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write('#\n')
        if top_plf:
            f.write(f'# Design Extreme (with PLF)    : '
                    f'{top_plf[0]:>12.4f}  {unit:<8}  '
                    f'PLF [{top_plf[1]:.2f}]     {top_plf[2]}\n')
        if top_noplf:
            f.write(f'# Design Extreme (without PLF) : '
...
def _infer_component_label(sensor_name):
    ...
    if any(p in n for p in ('mxc','mxb','mxt','mxp','mxa','mxs','mlxt','mxe')):
        return 'Mx'
    if any(p in n for p in ('myc','myb','myt','myp','mya','mys','mlyt','mye')):
        return 'My'
    if any(p in n for p in ('mzc','mzb','mzt','mzp','mza','mzs','mlzt','mze')):
        return 'Mz'
    # Forces
    if any(p in n for p in ('fxc','fxb','fxt','fxp','fxa','fxs','flxt')):
        return 'Fx'
    if any(p in n for p in ('fyc','fyb','fyt','fyp','fya','fys','flyt')):
        return 'Fy'
    if any(p in n for p in ('fzc','fzb','fzt','fzp','fza','fzs','flzt')):
        return 'Fz'
    return None


def write_complementary_file(output_path, sensor_col, stat,
                              comp_results, comp_method):
    """
    ...
    if not comp_results:
        ...
        print(f'  WARNING: No complementary results for {sensor_col} '
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'# CompMethod     : {method_desc.get(comp_method, str(comp_method))}\n')
        f.write(f'# Columns        : {" | ".join(lbl for lbl, _ in active_cols)} | PLF | Family | File | Time\n')
        ...
        if missing_labels:
            f.write(f'# Missing sensors : {", ".join(missing_labels)} '
        ...
        driving_label = _infer_component_label(
            comp_results[0]['driving_sensor']) if comp_results else None
...
def write_contribution_file(output_path, damage_per_file, file_metadata,
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write('#         Contribution_k = Damage_k * OccFreq_k / Total_damage * 100%\n')
        f.write('#\n')
        ...
        for m in m_values:
            ...
            total_dmg = {col: sum(weighted[f][col] for f in ordered_files)
                         for col in sensor_cols}
            ...
            for fname in ordered_files:
                ...
                row = f"  {fname:<{w_fname}}{occ:>{w_occ}.1f}"
                for col in sensor_cols:
                    pct = (weighted[fname][col] / total_dmg[col] * 100.0
                           if total_dmg[col] > 0 else 0.0)
                    row += fmt_pct(pct)
                f.write(row + '\n')
            f.write(f'# {subdiv}\n')
            tot_row = f"  {'TOTAL':<{w_fname}}{total_occ:>{w_occ}.1f}"
            ...
            for fam in family_order:
                row = f"  {fam:<{w_fam}}{fam_occ[fam]:>{w_occ}.1f}"
                for col in sensor_cols:
                    pct = (fam_dmg[fam][col] / total_dmg[col] * 100.0
                           if total_dmg[col] > 0 else 0.0)
                    row += fmt_pct(pct)
                f.write(row + '\n')
            f.write(f'# {fam_div}\n')
