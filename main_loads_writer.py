...
def _write_foundation(f, fnd_lines, fnd_cfg):
    ...
    def _write_fnd_ext(tbl_id, tbl_data, with_plf):
        ...
        for comp in COMPS_EXT:
            row = next((v for k, v in tbl_data.items()
                        if k.upper().startswith(comp + '_')), None)
            lbl = next((k for k in tbl_data
                        if k.upper().startswith(comp + '_')),
                       f'{comp}_[kN-m]')
            val_rows.append((lbl, row['val'] if row else None))
            plf  = row.get('plf') if row else None
            dlc  = row.get('dlc') if row else None
            plf_v = plf if plf else (1.35 if with_plf else 1.00)
            dlc_rows.append((lbl, f"{dlc or 'N/A'}  (PLF={plf_v:.2f})"))
        for lbl, val in val_rows:
            _vrow(f, lbl, val)
        _dotdiv(f)
        for lbl, dlc in dlc_rows:
