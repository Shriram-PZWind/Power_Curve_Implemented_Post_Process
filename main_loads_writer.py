...
from config import (
    get_blade_config,
    get_hub_config,
    get_pitch_bearing_config,
    get_drivetrain_config,
    get_yaw_config,
    get_tower_config,
    get_foundation_config,
    get_tower_clearance_config,
    fmt_slope,
)
...
def _read_lines(sum_folder, filename, log=None):
    ...
    if not os.path.isfile(path):
        if log:
            log.warning(
                f"{filename} not found in SUM folder — section skipped", tag="MainLoads"
            )
        return []
...
def _is_new_section_marker(line_stripped):
    ...
    return (
        upper_body.startswith("SECTION ")
        or upper_body.startswith("TABLE ")
        or upper_body == "END"
        or upper_body.startswith("END ")
    )
    ...
    if dlc_rows:
        for d_parts in dlc_rows:
            try:
                ...
                if "PLF=" in plf_str:
                    try:
                        # Extract the value, strip spaces, and convert to float
                        plf_val = float(
                            plf_str.split("PLF=")[1].replace(")", "").strip()
                        )
                    except (ValueError, IndexError):
                        # Fallback if the text after PLF= is not a valid number (e.g., "PLF=abc")
                        plf_val = 1.0
                    else:
                        # Instead of None, use 1.0 so the downstream formatter doesn't crash
                        plf_val = 1.0
...
def _parse_drt_table(lines, table_marker):
    ...
    for i in range(idx + 1, len(lines)):
        ...
        if "TABLE" in line.upper() and i > idx + 5:
            break
...
def _parse_yaw_section(lines, marker):
    ...
    def _parse_block(rows):
        return [
            {"label": p[0], "val": _val(p, 1), "plf": _val(p, 2), "dlc": _str(p, 3)}
            for p in rows
        ]

    return {
        "loads": _parse_block(blocks[0]),
...
def _write_blade(f, bld_lines, bld_cfg):
    ...
    f.write(
        f"# Stations : cubic spline interpolation at "
        f'{[f"{int(round(fr*100))}%" for fr in fractions]}\n'
    )
    f.write(f"# DLC      : from nearest spanwise station\n#\n")
    ...
    for tbl_id, sec_marker, comp, stat in EXT_TABLES:
        ...
        if sec and sec["radii"]:
            valid = [
                (r, v, d, b, p)
                for r, v, d, b, p in zip(
                    sec["radii"], sec["values"], sec["dlcs"], sec["blades"], sec["plfs"]
                )
                if r is not None
            ]
            if valid:
                ...
                plfs_v = [x[4] for x in valid]
                pts = interpolate_blade_loads(
                    radii_v, values_v, dlcs_v, blades_v, plfs_v, blade_length, fractions
                )
    ...
    for tbl_id, comp, sec_base in DEL_TABLES:
        ...
        for slope_idx, m in enumerate(del_slopes):
            ...
            if sec and sec["radii"]:
                valid = [
                    (r, v) for r, v in zip(sec["radii"], sec["values"]) if r is not None
                ]
                if valid:
                    rv = [x[0] for x in valid]
                    vv = [x[1] for x in valid]
    ...
    for comp in COMPS:
        ...
        row = next(
            (
                r
                for r in ext_rows
                if r["label"].strip().upper().startswith(comp.upper() + "_")
            ),
            None,
        )

        val_rows.append(
            (row["label"] if row else f"{comp}_[kN-m]", row["val"] if row else None)
        )
        plf_s = f"PLF={row['plf']:.2f}" if row and row["plf"] else "PLF=—"
        dlc_rows.append(
            (
                row["label"] if row else f"{comp}_[kN-m]",
...
def _write_pitch_drive(f, ptb_lines, ptb_cfg):
    ...
    def _write_ext_block(title, row_idx, with_plf):
        f.write(
            f'# TABLE 4{"A" if with_plf else "B"} — '
        ...
        _drow(f, lbl, f"{dlc or 'N/A'}  (PLF={plf_v:.2f})  {bld or ''}")
        _subdiv(f)
        f.write("#\n")
        return lbl
...
def _write_drivetrain(f, drt_lines, drt_cfg):
    ...
    _col_hdr(f)
    a_vals = _frame_rows(tbl1a, "a") or [{"label": f"{c}_a", "cols": []} for c in COMPS]
    s_vals = _frame_rows(tbl1a, "s") or [{"label": f"{c}_s", "cols": []} for c in COMPS]
    ...
    for i, m in enumerate(del_slopes):
        f.write(
            f"# ── Non-rotating frame (a suffix)  m = {fmt_slope(m)} ─────────────────────\n"
        )
        for r in a_del:
            _vrow(f, r["label"], _val(r["cols"], i))
        f.write(
            f"# ── Rotating frame (s suffix)  m = {fmt_slope(m)} ──────────────────────────\n"
        )
        for r in s_del:
            _vrow(f, r["label"], _val(r["cols"], i))
    _subdiv(f)
    ...
    a_ldd = [
        r
        for r in ldd_my_mz
        if "a_[" in r["label"].lower() or r["label"].lower().split("_")[0].endswith("a")
    ]
    s_ldd = [
        r
        for r in ldd_my_mz
        if "s_[" in r["label"].lower() or r["label"].lower().split("_")[0].endswith("s")
    ]

    for ldd_i, ldd_m in enumerate(ldd_slopes):
        f.write(
            f"# ── Non-rotating (a)  LDD m = {fmt_slope(ldd_m)} ──────────────────────────────\n"
        )
        for r in a_ldd:
            _vrow(f, r["label"], _val(r["cols"], ldd_i))
        f.write(
            f"# ── Rotating (s)  LDD m = {fmt_slope(ldd_m)} ─────────────────────────────────\n"
        )
        for r in s_ldd:
            _vrow(f, r["label"], _val(r["cols"], ldd_i))
    for lrd_i, lrd_m in enumerate(lrd_slopes):
        ...
        f.write(
            f"# ── Non-rotating (a)  LRD m = {fmt_slope(lrd_m)} ──────────────────────────────\n"
        )
        for r in a_ldd:
            _vrow(f, r["label"], _val(r["cols"], n_ldd + lrd_i))
        f.write(
            f"# ── Rotating (s)  LRD m = {fmt_slope(lrd_m)} ─────────────────────────────────\n"
        )
        for r in s_ldd:
            _vrow(f, r["label"], _val(r["cols"], n_ldd + lrd_i))
    _subdiv(f)
...
def _write_foundation(f, fnd_lines, fnd_cfg):
    ...
    def _write_fnd_ext(tbl_id, tbl_data, with_plf):
        ...
        for comp in COMPS_EXT:
            ...
            dlc_rows.append(
                (lbl, f"{dlc}  (PLF={f'{plf:.2f}' if plf is not None else 'N/A'})")
            )
        for lbl, val in val_rows:
            _vrow(f, lbl, val)
        _dotdiv(f)
            ...
            for comp in COMPS_FAT:
                ...
                lbl, cols = next(
                    (
                        ...
                        if comp.upper() + "_" in k.upper()
                        or (comp.upper() + "T") in k.upper()
                    ),
                    (f"{comp}_[kN-m]", None),
                )
...
def _write_clearance(f, ext_folder, clr_cfg):
        ...
        for b_key in ["B1", "B2", "B3"]:
            ...
            if sensor and ext_folder:
                val, plf, dlc = read_ext_design_load(
                    ext_folder, sensor, ext_type, with_plf=False
                )
            else:
                val = plf = dlc = None
            val_rows.append((sensor or f"{label_prefix}_{b_key}_[m]", b_lbl, val))
            dlc_rows.append(
                (sensor or f"{label_prefix}_{b_key}_[m]", b_lbl, dlc or "N/A")
            )
            # Track worst
            if val is not None:
                if ext_type == "min":
    ...
    f.write(
        "# ── Direct clearance (TwrClrnc) — Min value ─────────────────────────────\n"
    )
    ...
    # OoP deflection — Max
    f.write(
        "# ── Out-of-plane tip deflection (OoPDefl) — Max value ───────────────────\n"
    )
...
def write_main_loads_sum(
    output_path, sum_folder, ext_folder, lifetime_years, neq_lifetime, log=None
):
    """
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"# Note      : All slopes read from sensorList.txt — nothing hardcoded\n"
        )
        f.write(f'# {"=" * 76}\n')
